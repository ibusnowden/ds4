...
static void usage(FILE *fp) {
    fprintf(fp,
        ...
        "  --backend NAME\n"
        "      Select backend explicitly: metal, cuda, or cpu. Default: metal\n"
        "  -t, --threads N\n"
...
int main(int argc, char **argv) {
    ...
    if (rc == 0 && cfg.session_smoke) {
        ds4_session *session = NULL;
        if (ds4_session_create(&session, engine, cfg.gen.ctx_size) != 0) {
            fprintf(stderr, "ds4: session smoke failed for backend %s\n",
                    ds4_backend_name(cfg.engine.backend));
            rc = 1;
        } else {
            fprintf(stderr, "ds4: session smoke ok for backend %s (ctx=%d)\n",
                    ds4_backend_name(cfg.engine.backend),
                    cfg.gen.ctx_size);
            ds4_session_free(session);
        }
