...
typedef struct {
    ...
    ds4_cuda_tensor *probe_flat_hc;
    ds4_cuda_tensor *probe_hc_mix;
    ds4_cuda_tensor *probe_hc_split;
    ds4_cuda_tensor *probe_attn_cur;
    ds4_cuda_tensor *probe_attn_norm;
    ds4_cuda_tensor *probe_qr;
    ds4_cuda_tensor *probe_qr_norm;
    ds4_cuda_tensor *probe_q;
    ds4_cuda_tensor *probe_kv_raw;
    ds4_cuda_tensor *probe_kv;
    ds4_cuda_tensor *probe_raw_cache;
    ds4_cuda_tensor *probe_heads;
    ds4_cuda_tensor *probe_attn_low;
    ds4_cuda_tensor *probe_attn_out;
    ds4_cuda_tensor *probe_ffn_hc_mix;
    ds4_cuda_tensor *probe_ffn_hc_split;
    ds4_cuda_tensor *probe_ffn_cur;
    ds4_cuda_tensor *probe_ffn_norm;
    ds4_cuda_tensor *probe_shared_gate;
    ds4_cuda_tensor *probe_shared_up;
    ds4_cuda_tensor *probe_shared_mid;
    ds4_cuda_tensor *probe_shared_out;
    ds4_cuda_tensor *probe_routed_mid;
    ds4_cuda_tensor *probe_routed_out;
    ds4_cuda_tensor *probe_ffn_out;
    ds4_cuda_tensor *probe_router_logits;
    ds4_cuda_tensor *probe_router_probs;
    ds4_cuda_tensor *probe_router_selected;
    ds4_cuda_tensor *probe_router_weights;
    ds4_cuda_tensor *probe_after_ffn_hc;
    ds4_cuda_host_map *model_weights;
    uint64_t model_device_base;
    uint64_t model_weight_bytes;
    ds4_cuda_module *module;
    ds4_cuda_kernel *clear_f32;
    ds4_cuda_kernel *embed_token_hc_f16;
    ...
    ds4_cuda_kernel *add_f32;
    ds4_cuda_kernel *routed_iq2_swiglu_f32;
    ds4_cuda_kernel *routed_q2_down_sum_f32;
    ds4_cuda_kernel *router_hash_select_f32;
    ds4_cuda_kernel *hc4_split_weighted_sum_norm_f32;
    ds4_cuda_kernel *rms_norm_f32;
    ds4_cuda_kernel *head_rms_norm_f32;
...
static void cuda_graph_free(ds4_cuda_graph *g) {
    ...
    ds4_cuda_kernel_free(g->head_rms_norm_f32);
    ds4_cuda_kernel_free(g->rms_norm_f32);
    ds4_cuda_kernel_free(g->hc4_split_weighted_sum_norm_f32);
    ds4_cuda_kernel_free(g->hc4_post_f32);
    ds4_cuda_kernel_free(g->swiglu_f32);
    ds4_cuda_kernel_free(g->routed_q2_down_sum_f32);
    ds4_cuda_kernel_free(g->routed_iq2_swiglu_f32);
    ds4_cuda_kernel_free(g->add_f32);
    ...
    ds4_cuda_kernel_free(g->clear_f32);
    ds4_cuda_module_free(g->module);
    ds4_cuda_host_unregister(g->model_weights);
    ds4_cuda_tensor_free(g->probe_kv);
    ds4_cuda_tensor_free(g->probe_raw_cache);
    ds4_cuda_tensor_free(g->probe_attn_out);
    ds4_cuda_tensor_free(g->probe_attn_low);
    ds4_cuda_tensor_free(g->probe_heads);
    ds4_cuda_tensor_free(g->probe_after_ffn_hc);
    ds4_cuda_tensor_free(g->probe_router_weights);
    ds4_cuda_tensor_free(g->probe_router_selected);
    ds4_cuda_tensor_free(g->probe_router_probs);
    ds4_cuda_tensor_free(g->probe_router_logits);
    ds4_cuda_tensor_free(g->probe_shared_out);
    ds4_cuda_tensor_free(g->probe_ffn_out);
    ds4_cuda_tensor_free(g->probe_routed_out);
    ds4_cuda_tensor_free(g->probe_routed_mid);
    ds4_cuda_tensor_free(g->probe_shared_mid);
    ds4_cuda_tensor_free(g->probe_shared_up);
    ds4_cuda_tensor_free(g->probe_shared_gate);
    ds4_cuda_tensor_free(g->probe_ffn_norm);
    ds4_cuda_tensor_free(g->probe_ffn_cur);
    ds4_cuda_tensor_free(g->probe_ffn_hc_split);
    ds4_cuda_tensor_free(g->probe_ffn_hc_mix);
    ds4_cuda_tensor_free(g->probe_kv_raw);
    ds4_cuda_tensor_free(g->probe_q);
    ds4_cuda_tensor_free(g->probe_qr_norm);
    ds4_cuda_tensor_free(g->probe_qr);
    ds4_cuda_tensor_free(g->probe_attn_norm);
    ds4_cuda_tensor_free(g->probe_attn_cur);
    ds4_cuda_tensor_free(g->probe_hc_split);
    ds4_cuda_tensor_free(g->probe_hc_mix);
    ds4_cuda_tensor_free(g->probe_flat_hc);
    ds4_cuda_tensor_free(g->logits);
    ds4_cuda_tensor_free(g->next_hc);
    ds4_cuda_tensor_free(g->cur_hc);
    ds4_cuda_tensor_free(g->tokens);
    memset(g, 0, sizeof(*g));
}

static uint32_t cuda_graph_prefill_cap_for_context(uint32_t ctx_size) {
    if (ctx_size >= 4096u) return 512u;
    if (ctx_size >= 1024u) return 256u;
    return ctx_size ? ctx_size : 1u;
}

static uint32_t cuda_graph_comp_cap_for_context(uint32_t ctx_size) {
    uint32_t min_ratio = UINT32_MAX;
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        const uint32_t ratio = ds4_layer_compress_ratio(il);
        if (ratio != 0 && ratio < min_ratio) min_ratio = ratio;
    ...
    if (min_ratio == UINT32_MAX) min_ratio = ctx_size ? ctx_size : 1u;
    uint32_t cap = ctx_size / min_ratio + 2u;
    return cap < 2u ? 2u : cap;
}

static bool cuda_graph_load_executor(ds4_cuda_graph *g, char *err, size_t errlen) {
    if (!ds4_cuda_module_load_source(&g->module,
                                     ds4_cuda_kernel_source(),
                                     "ds4_cuda_kernels.cu",
                                     err,
                                     errlen))
    {
    ...
    if (!ds4_cuda_module_get_kernel(&g->clear_f32, g->module, "ds4_clear_f32", err, errlen) ||
        ...
        !ds4_cuda_module_get_kernel(&g->swiglu_f32, g->module, "ds4_swiglu_f32", err, errlen) ||
        !ds4_cuda_module_get_kernel(&g->add_f32, g->module, "ds4_add_f32", err, errlen) ||
        !ds4_cuda_module_get_kernel(&g->routed_iq2_swiglu_f32, g->module, "ds4_routed_iq2_swiglu_f32", err, errlen) ||
        !ds4_cuda_module_get_kernel(&g->routed_q2_down_sum_f32, g->module, "ds4_routed_q2_down_sum_f32", err, errlen) ||
        !ds4_cuda_module_get_kernel(&g->router_hash_select_f32, g->module, "ds4_router_hash_select_f32", err, errlen) ||
        !ds4_cuda_module_get_kernel(&g->hc4_split_weighted_sum_norm_f32, g->module, "ds4_hc4_split_weighted_sum_norm_f32", err, errlen) ||
        !ds4_cuda_module_get_kernel(&g->rms_norm_f32, g->module, "ds4_rms_norm_f32", err, errlen) ||
    ...
    g->executor_ready = true;
    return true;
}

static bool cuda_graph_zero_logits(ds4_cuda_graph *g, char *err, size_t errlen) {
    if (!g || !g->executor_ready || !g->clear_f32 || !g->logits) {
        snprintf(err, errlen, "CUDA graph executor is not ready");
        return false;
    }
    uint64_t ptr = ds4_cuda_tensor_device_ptr(g->logits);
    uint32_t n = DS4_N_VOCAB;
    void *args[] = { &ptr, &n };
    const uint32_t block = 256;
    const uint32_t grid = (n + block - 1u) / block;
    return ds4_cuda_launch_kernel(g->clear_f32, grid, 1, 1, block, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_register_model_weights(ds4_cuda_graph *g,
                                              const ds4_model *model,
                                              char *err,
                                              size_t errlen) {
    if (!model || !model->map || model->tensor_data_pos >= model->size) {
        snprintf(err, errlen, "CUDA model weight map is invalid");
        return false;
    }
    const uint8_t *weights = model->map + model->tensor_data_pos;
    const uint64_t bytes = model->size - model->tensor_data_pos;
    if (!ds4_cuda_host_register(&g->model_weights, weights, bytes, err, errlen)) {
        return false;
    }
...
static bool cuda_graph_embed_token(ds4_cuda_graph *g,
                                   ...
                                   char *err,
                                   size_t errlen) {
    if (!g || !g->executor_ready || !g->embed_token_hc_f16 || !g->cur_hc ||
        !model || !weights || !weights->token_embd)
    {
        snprintf(err, errlen, "CUDA token embedding kernel is not ready");
        return false;
    }
    if (token < 0 || (uint64_t)token >= weights->token_embd->dim[1]) {
        snprintf(err, errlen, "CUDA token id is outside the embedding table");
        return false;
    }
    if (weights->token_embd->abs_offset < model->tensor_data_pos ||
        weights->token_embd->abs_offset - model->tensor_data_pos >= g->model_weight_bytes)
    {
        snprintf(err, errlen, "CUDA token embedding tensor is outside the mapped weight range");
        return false;
    }

    uint64_t token_embd_ptr =
        g->model_device_base + (weights->token_embd->abs_offset - model->tensor_data_pos);
    uint64_t dst_ptr = ds4_cuda_tensor_device_ptr(g->cur_hc);
    ...
    const uint32_t block = 256;
    const uint32_t grid = (n + block - 1u) / block;
    return ds4_cuda_launch_kernel(g->embed_token_hc_f16, grid, 1, 1, block, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_tensor_device_ptr(const ds4_cuda_graph *g,
                                         const ds4_model *model,
                                         const ds4_tensor *tensor,
                                         uint64_t *out,
                                         char *err,
                                         size_t errlen) {
    if (!g || !model || !tensor || !out) {
        snprintf(err, errlen, "CUDA tensor device pointer request is invalid");
        return false;
    }
    if (tensor->abs_offset < model->tensor_data_pos ||
        tensor->abs_offset - model->tensor_data_pos > g->model_weight_bytes ||
        tensor->bytes > g->model_weight_bytes - (tensor->abs_offset - model->tensor_data_pos))
    {
...
static bool cuda_graph_launch_rms(ds4_cuda_graph *g,
                                  ds4_cuda_tensor *dst,
                                  ds4_cuda_tensor *src,
                                  uint64_t weight_ptr,
                                  uint32_t n,
    ...
    return ds4_cuda_launch_kernel(g->rms_norm_f32, 1, 1, 1, 256, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_matvec_f16(ds4_cuda_graph *g,
                                         ds4_cuda_tensor *dst,
                                         uint64_t weight_ptr,
                                         ds4_cuda_tensor *x,
                                         uint32_t in_dim,
    ...
    void *args[] = { &dst_ptr, &weight_ptr, &x_ptr, &in_dim, &out_dim };
    return ds4_cuda_launch_kernel(g->matvec_f16_f32, out_dim, 1, 1, 256, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_matvec_q8_0(ds4_cuda_graph *g,
                                          ds4_cuda_tensor *dst,
                                          uint64_t weight_ptr,
                                          ds4_cuda_tensor *x,
                                          uint32_t in_dim,
    ...
    uint64_t x_ptr = ds4_cuda_tensor_device_ptr(x);
    void *args[] = { &dst_ptr, &weight_ptr, &x_ptr, &in_dim, &out_dim };
    return ds4_cuda_launch_kernel(g->matvec_q8_0_f32, out_dim, 1, 1, 256, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_matvec_q8_0_grouped(ds4_cuda_graph *g,
                                                  ds4_cuda_tensor *dst,
                                                  uint64_t weight_ptr,
                                                  ds4_cuda_tensor *x,
                                                  uint32_t group_dim,
    ...
    uint64_t dst_ptr = ds4_cuda_tensor_device_ptr(dst);
    uint64_t x_ptr = ds4_cuda_tensor_device_ptr(x);
    void *args[] = { &dst_ptr, &weight_ptr, &x_ptr, &group_dim, &rank, &n_groups };
    return ds4_cuda_launch_kernel(g->matvec_q8_0_grouped_f32,
                                  rank * n_groups, 1, 1, 256, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_rope(ds4_cuda_graph *g,
                                   ds4_cuda_tensor *x,
                                   uint32_t n_head,
                                   uint32_t head_dim,
                                   uint32_t n_rot,
    ...
    const uint32_t block = 256;
    const uint32_t grid = (n + block - 1u) / block;
    return ds4_cuda_launch_kernel(g->rope_tail_f32, grid, 1, 1, block, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_attention_one(ds4_cuda_graph *g,
                                            uint64_t sinks_ptr,
                                            char *err,
                                            size_t errlen) {
    uint64_t heads_ptr = ds4_cuda_tensor_device_ptr(g->probe_heads);
    uint64_t q_ptr = ds4_cuda_tensor_device_ptr(g->probe_q);
    uint64_t kv_ptr = ds4_cuda_tensor_device_ptr(g->probe_kv);
    uint32_t n_head = DS4_N_HEAD;
    uint32_t head_dim = DS4_N_HEAD_DIM;
    void *args[] = { &heads_ptr, &q_ptr, &kv_ptr, &sinks_ptr, &n_head, &head_dim };
    return ds4_cuda_launch_kernel(g->attention_one_f32, n_head, 1, 1, 256, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_hc4_post(ds4_cuda_graph *g, char *err, size_t errlen) {
    uint64_t dst_ptr = ds4_cuda_tensor_device_ptr(g->next_hc);
    uint64_t block_ptr = ds4_cuda_tensor_device_ptr(g->probe_attn_out);
    uint64_t residual_ptr = ds4_cuda_tensor_device_ptr(g->cur_hc);
    uint64_t split_ptr = ds4_cuda_tensor_device_ptr(g->probe_hc_split);
    uint32_t n_embd = DS4_N_EMBD;
    void *args[] = { &dst_ptr, &block_ptr, &residual_ptr, &split_ptr, &n_embd };
    const uint32_t block = 256;
    const uint32_t grid = (n_embd + block - 1u) / block;
    return ds4_cuda_launch_kernel(g->hc4_post_f32, grid, 1, 1, block, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_hc4_post_to(ds4_cuda_graph *g,
                                          ds4_cuda_tensor *dst,
...
static bool cuda_graph_launch_swiglu(ds4_cuda_graph *g,
                                     ds4_cuda_tensor *dst,
    ...
    const uint32_t block = 256;
    const uint32_t grid = (n + block - 1u) / block;
    return ds4_cuda_launch_kernel(g->swiglu_f32, grid, 1, 1, block, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_add(ds4_cuda_graph *g,
                                  ds4_cuda_tensor *dst,
                                  ds4_cuda_tensor *a,
                                  ds4_cuda_tensor *b,
                                  uint32_t n,
    ...
    const uint32_t block = 256;
    const uint32_t grid = (n + block - 1u) / block;
    return ds4_cuda_launch_kernel(g->add_f32, grid, 1, 1, block, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_routed_iq2_swiglu(ds4_cuda_graph *g,
                                                uint64_t gate_ptr,
                                                uint64_t up_ptr,
                                                ...
                                                char *err,
                                                size_t errlen) {
    uint64_t mid_ptr = ds4_cuda_tensor_device_ptr(g->probe_routed_mid);
    uint64_t x_ptr = ds4_cuda_tensor_device_ptr(g->probe_ffn_norm);
    uint64_t selected_ptr = ds4_cuda_tensor_device_ptr(g->probe_router_selected);
    uint64_t weights_ptr = ds4_cuda_tensor_device_ptr(g->probe_router_weights);
    ...
    return ds4_cuda_launch_kernel(g->routed_iq2_swiglu_f32,
                                  mid_dim, n_used, 1, 1, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_routed_q2_down_sum(ds4_cuda_graph *g,
                                                 uint64_t down_ptr,
                                                 uint64_t down_expert_bytes,
                                                 uint64_t down_row_bytes,
                                                 char *err,
                                                 size_t errlen) {
    uint64_t out_ptr = ds4_cuda_tensor_device_ptr(g->probe_routed_out);
    uint64_t mid_ptr = ds4_cuda_tensor_device_ptr(g->probe_routed_mid);
    uint64_t selected_ptr = ds4_cuda_tensor_device_ptr(g->probe_router_selected);
    ...
    return ds4_cuda_launch_kernel(g->routed_q2_down_sum_f32,
                                  out_dim, 1, 1, 1, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_router_hash(ds4_cuda_graph *g,
                                          uint64_t tid2eid_ptr,
                                          uint32_t token,
                                          uint32_t n_vocab,
                                          char *err,
                                          size_t errlen) {
    uint64_t probs_ptr = ds4_cuda_tensor_device_ptr(g->probe_router_probs);
    uint64_t selected_ptr = ds4_cuda_tensor_device_ptr(g->probe_router_selected);
    uint64_t weights_ptr = ds4_cuda_tensor_device_ptr(g->probe_router_weights);
    uint64_t logits_ptr = ds4_cuda_tensor_device_ptr(g->probe_router_logits);
    ...
    return ds4_cuda_launch_kernel(g->router_hash_select_f32, 1, 1, 1, 256, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_hc4_attn_pre_norm(ds4_cuda_graph *g,
                                                uint64_t scale_ptr,
                                                uint64_t base_ptr,
                                                uint64_t norm_weight_ptr,
                                                char *err,
                                                size_t errlen) {
    uint64_t dst_ptr = ds4_cuda_tensor_device_ptr(g->probe_attn_cur);
    uint64_t norm_dst_ptr = ds4_cuda_tensor_device_ptr(g->probe_attn_norm);
    uint64_t split_ptr = ds4_cuda_tensor_device_ptr(g->probe_hc_split);
    uint64_t mix_ptr = ds4_cuda_tensor_device_ptr(g->probe_hc_mix);
    uint64_t x_ptr = ds4_cuda_tensor_device_ptr(g->cur_hc);
    uint32_t n_embd = DS4_N_EMBD;
    ...
    return ds4_cuda_launch_kernel(g->hc4_split_weighted_sum_norm_f32, 1, 1, 1, 256, 1, 1, 0, args, err, errlen) != 0 &&
           ds4_cuda_synchronize(err, errlen) != 0;
}

static bool cuda_graph_launch_hc4_split_norm_to(ds4_cuda_graph *g,
                                                ds4_cuda_tensor *dst,
    ...
    uint64_t x_ptr = ds4_cuda_tensor_device_ptr(residual_hc);
    uint32_t n_embd = DS4_N_EMBD;
    uint32_t sinkhorn_iters = DS4_N_HC_SINKHORN_ITER;
    float eps = DS4_HC_EPS;
    float norm_eps = DS4_RMS_EPS;
...
static bool cuda_graph_probe_layer0_q_path(ds4_cuda_graph *g,
                                           const ds4_model *model,
                                           const ds4_weights *weights,
                                           int token,
                                           char *err,
    ...
    uint64_t w_hc = 0;
    uint64_t w_hc_scale = 0;
    uint64_t w_hc_base = 0;
    uint64_t w_attn_norm = 0;
    uint64_t w_q_a = 0;
    uint64_t w_q_a_norm = 0;
    uint64_t w_q_b = 0;
    uint64_t w_kv = 0;
    uint64_t w_kv_norm = 0;
    uint64_t w_sinks = 0;
    uint64_t w_out_a = 0;
    uint64_t w_out_b = 0;
    uint64_t w_ffn_hc = 0;
    uint64_t w_ffn_hc_scale = 0;
    uint64_t w_ffn_hc_base = 0;
    uint64_t w_ffn_norm = 0;
    uint64_t w_shared_gate = 0;
    uint64_t w_shared_up = 0;
    uint64_t w_shared_down = 0;
    uint64_t w_routed_gate = 0;
    uint64_t w_routed_up = 0;
    uint64_t w_routed_down = 0;
    uint64_t w_router = 0;
    uint64_t w_tid2eid = 0;

    if (!cuda_graph_tensor_device_ptr(g, model, layer->hc_attn_fn, &w_hc, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->hc_attn_scale, &w_hc_scale, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->hc_attn_base, &w_hc_base, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_norm, &w_attn_norm, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_q_a, &w_q_a, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_q_a_norm, &w_q_a_norm, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_q_b, &w_q_b, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_kv, &w_kv, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_kv_a_norm, &w_kv_norm, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_sinks, &w_sinks, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_output_a, &w_out_a, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->attn_output_b, &w_out_b, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->hc_ffn_fn, &w_ffn_hc, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->hc_ffn_scale, &w_ffn_hc_scale, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->hc_ffn_base, &w_ffn_hc_base, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_norm, &w_ffn_norm, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_gate_shexp, &w_shared_gate, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_up_shexp, &w_shared_up, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_down_shexp, &w_shared_down, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_gate_exps, &w_routed_gate, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_up_exps, &w_routed_up, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_down_exps, &w_routed_down, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_gate_inp, &w_router, err, errlen) ||
        !cuda_graph_tensor_device_ptr(g, model, layer->ffn_gate_tid2eid, &w_tid2eid, err, errlen))
    {
        return false;
    }
    ...
    {
        snprintf(err, errlen, "CUDA routed MoE currently supports IQ2_XXS gate/up and Q2_K down experts");
        return false;
    }
    const uint64_t gate_row_bytes = routed_expert_row_bytes(layer->ffn_gate_exps);
    ...
    if (!cuda_graph_launch_rms(g, g->probe_flat_hc, g->cur_hc, 0, hc_dim, false, err, errlen) ||
        !cuda_graph_launch_matvec_f16(g, g->probe_hc_mix, w_hc, g->probe_flat_hc, hc_dim, hc_mix, err, errlen) ||
        !cuda_graph_launch_hc4_attn_pre_norm(g, w_hc_scale, w_hc_base, w_attn_norm, err, errlen) ||
        !cuda_graph_launch_matvec_q8_0(g, g->probe_qr, w_q_a, g->probe_attn_norm,
                                       DS4_N_EMBD, DS4_N_LORA_Q, err, errlen) ||
        !cuda_graph_launch_rms(g, g->probe_qr_norm, g->probe_qr, w_q_a_norm, DS4_N_LORA_Q, true, err, errlen) ||
        !cuda_graph_launch_matvec_q8_0(g, g->probe_q, w_q_b, g->probe_qr_norm,
                                       DS4_N_LORA_Q, q_dim, err, errlen) ||
        !cuda_graph_launch_matvec_q8_0(g, g->probe_kv_raw, w_kv, g->probe_attn_norm,
                                       DS4_N_EMBD, DS4_N_HEAD_DIM, err, errlen) ||
        !cuda_graph_launch_rms(g, g->probe_kv, g->probe_kv_raw, w_kv_norm, DS4_N_HEAD_DIM, true, err, errlen))
    {
        return false;
    }
    ...
    uint32_t head_dim = DS4_N_HEAD_DIM;
    float eps = DS4_RMS_EPS;
    void *head_args[] = { &q_ptr, &n_head, &head_dim, &eps };
    if (ds4_cuda_launch_kernel(g->head_rms_norm_f32, n_head, 1, 1, 256, 1, 1, 0, head_args, err, errlen) == 0 ||
        ds4_cuda_synchronize(err, errlen) == 0)
    {
        return false;
    }
    if (!cuda_graph_launch_rope(g, g->probe_q, DS4_N_HEAD, DS4_N_HEAD_DIM, DS4_N_ROT, 0, 0, false, err, errlen) ||
        !cuda_graph_launch_rope(g, g->probe_kv, DS4_N_HEAD_KV, DS4_N_HEAD_DIM, DS4_N_ROT, 0, 0, false, err, errlen))
    {
        return false;
    }
    if (!ds4_cuda_tensor_copy(g->probe_raw_cache, 0, g->probe_kv, 0,
                              (uint64_t)DS4_N_HEAD_DIM * sizeof(float)))
    {
        snprintf(err, errlen, "CUDA layer-0 KV raw-cache store failed");
        return false;
    }
    if (!cuda_graph_launch_attention_one(g, w_sinks, err, errlen) ||
        !cuda_graph_launch_rope(g, g->probe_heads, DS4_N_HEAD, DS4_N_HEAD_DIM, DS4_N_ROT, 0, 0, true, err, errlen) ||
        !cuda_graph_launch_matvec_q8_0_grouped(g,
                                               g->probe_attn_low,
                                               w_out_a,
                                               g->probe_heads,
                                               ...
                                               err,
                                               errlen) ||
        !cuda_graph_launch_matvec_q8_0(g, g->probe_attn_out, w_out_b, g->probe_attn_low,
                                       DS4_N_OUT_GROUP * DS4_N_LORA_O, DS4_N_EMBD, err, errlen) ||
        !cuda_graph_launch_hc4_post(g, err, errlen))
    {
        return false;
    }
    if (!cuda_graph_launch_rms(g, g->probe_flat_hc, g->next_hc, 0, hc_dim, false, err, errlen) ||
        !cuda_graph_launch_matvec_f16(g, g->probe_ffn_hc_mix, w_ffn_hc, g->probe_flat_hc, hc_dim, hc_mix, err, errlen) ||
        !cuda_graph_launch_hc4_split_norm_to(g,
                                             g->probe_ffn_cur,
                                             g->probe_ffn_norm,
                                             g->probe_ffn_hc_split,
                                             g->probe_ffn_hc_mix,
                                             g->next_hc,
                                             w_ffn_hc_scale,
                                             w_ffn_hc_base,
                                             ...
                                             err,
                                             errlen) ||
        !cuda_graph_launch_matvec_f16(g, g->probe_router_logits, w_router, g->probe_ffn_norm,
                                      DS4_N_EMBD, DS4_N_EXPERT, err, errlen) ||
        !cuda_graph_launch_router_hash(g, w_tid2eid, (uint32_t)token, DS4_N_VOCAB, err, errlen) ||
        !cuda_graph_launch_routed_iq2_swiglu(g,
                                             w_routed_gate,
                                             w_routed_up,
                                             gate_expert_bytes,
                                             gate_row_bytes,
                                             ...
                                             err,
                                             errlen) ||
        !cuda_graph_launch_routed_q2_down_sum(g,
                                              w_routed_down,
                                              down_expert_bytes,
                                              down_row_bytes,
                                              err,
                                              errlen) ||
        !cuda_graph_launch_matvec_q8_0(g, g->probe_shared_gate, w_shared_gate, g->probe_ffn_norm,
                                       DS4_N_EMBD, DS4_N_FF_EXP, err, errlen) ||
        !cuda_graph_launch_matvec_q8_0(g, g->probe_shared_up, w_shared_up, g->probe_ffn_norm,
                                       DS4_N_EMBD, DS4_N_FF_EXP, err, errlen) ||
        !cuda_graph_launch_swiglu(g, g->probe_shared_mid, g->probe_shared_gate, g->probe_shared_up,
                                  DS4_N_FF_EXP, 0.0f, 1.0f, err, errlen) ||
        !cuda_graph_launch_matvec_q8_0(g, g->probe_shared_out, w_shared_down, g->probe_shared_mid,
                                       DS4_N_FF_EXP, DS4_N_EMBD, err, errlen) ||
        !cuda_graph_launch_add(g,
                               g->probe_ffn_out,
                               g->probe_shared_out,
                               g->probe_routed_out,
                               DS4_N_EMBD,
                               err,
                               errlen) ||
        !cuda_graph_launch_hc4_post_to(g,
...
static bool cuda_graph_alloc(ds4_cuda_graph *g,
    ...
    if (ctx_size == 0) ctx_size = 1;
    g->ctx_size = ctx_size;
    g->prefill_cap = cuda_graph_prefill_cap_for_context(ctx_size);
    ...
    const uint64_t q_dim = (uint64_t)DS4_N_HEAD * DS4_N_HEAD_DIM;
    const uint64_t attn_low = (uint64_t)DS4_N_OUT_GROUP * DS4_N_LORA_O;
    g->tokens = ds4_cuda_tensor_alloc((uint64_t)g->prefill_cap * sizeof(int32_t));
    g->cur_hc = ds4_cuda_tensor_alloc(hc_bytes);
    g->next_hc = ds4_cuda_tensor_alloc(hc_bytes);
    g->logits = ds4_cuda_tensor_alloc((uint64_t)DS4_N_VOCAB * sizeof(float));
    g->probe_flat_hc = ds4_cuda_tensor_alloc(hc_bytes);
    g->probe_hc_mix = ds4_cuda_tensor_alloc(hc_mix * sizeof(float));
    g->probe_hc_split = ds4_cuda_tensor_alloc(hc_mix * sizeof(float));
    g->probe_attn_cur = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EMBD * sizeof(float));
    g->probe_attn_norm = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EMBD * sizeof(float));
    g->probe_qr = ds4_cuda_tensor_alloc((uint64_t)DS4_N_LORA_Q * sizeof(float));
    g->probe_qr_norm = ds4_cuda_tensor_alloc((uint64_t)DS4_N_LORA_Q * sizeof(float));
    g->probe_q = ds4_cuda_tensor_alloc(q_dim * sizeof(float));
    g->probe_kv_raw = ds4_cuda_tensor_alloc((uint64_t)DS4_N_HEAD_DIM * sizeof(float));
    g->probe_kv = ds4_cuda_tensor_alloc((uint64_t)DS4_N_HEAD_DIM * sizeof(float));
    g->probe_raw_cache = ds4_cuda_tensor_alloc((uint64_t)DS4_N_HEAD_DIM * sizeof(float));
    g->probe_heads = ds4_cuda_tensor_alloc(q_dim * sizeof(float));
    g->probe_attn_low = ds4_cuda_tensor_alloc(attn_low * sizeof(float));
    g->probe_attn_out = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EMBD * sizeof(float));
    g->probe_ffn_hc_mix = ds4_cuda_tensor_alloc(hc_mix * sizeof(float));
    g->probe_ffn_hc_split = ds4_cuda_tensor_alloc(hc_mix * sizeof(float));
    g->probe_ffn_cur = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EMBD * sizeof(float));
    g->probe_ffn_norm = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EMBD * sizeof(float));
    g->probe_shared_gate = ds4_cuda_tensor_alloc((uint64_t)DS4_N_FF_EXP * sizeof(float));
    g->probe_shared_up = ds4_cuda_tensor_alloc((uint64_t)DS4_N_FF_EXP * sizeof(float));
    g->probe_shared_mid = ds4_cuda_tensor_alloc((uint64_t)DS4_N_FF_EXP * sizeof(float));
    g->probe_shared_out = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EMBD * sizeof(float));
    g->probe_routed_mid = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EXPERT_USED * DS4_N_FF_EXP * sizeof(float));
    g->probe_routed_out = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EMBD * sizeof(float));
    g->probe_ffn_out = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EMBD * sizeof(float));
    g->probe_router_logits = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EXPERT * sizeof(float));
    g->probe_router_probs = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EXPERT * sizeof(float));
    g->probe_router_selected = ds4_cuda_tensor_alloc((uint64_t)DS4_N_EXPERT_USED * sizeof(int32_t));
    ...
    const bool ok = g->tokens && g->cur_hc && g->next_hc && g->logits &&
                    ...
                    g->probe_qr && g->probe_qr_norm && g->probe_q &&
                    g->probe_kv_raw && g->probe_kv && g->probe_raw_cache &&
                    g->probe_heads && g->probe_attn_low && g->probe_attn_out &&
                    g->probe_ffn_hc_mix && g->probe_ffn_hc_split &&
                    g->probe_ffn_cur && g->probe_ffn_norm &&
                    g->probe_shared_gate && g->probe_shared_up &&
                    g->probe_shared_mid && g->probe_shared_out &&
    ...
    if (!cuda_graph_register_model_weights(g, model, err, errlen)) {
        cuda_graph_free(g);
        return false;
    }
    if (!cuda_graph_load_executor(g, err, errlen) ||
        !cuda_graph_zero_logits(g, err, errlen))
    {
        cuda_graph_free(g);
        return false;
    }
...
static int cuda_graph_unimplemented(char *err, size_t errlen, const char *op) {
    snprintf(err, errlen,
             "CUDA %s requested, but DS4 CUDA graph kernels are not implemented yet. "
             "Runtime, tensor allocation, copies, module loading, and kernel launch wrappers exist; "
             "the model graph still needs CUDA ports of the Metal kernels.",
             op);
    return 1;
}
#endif
...
struct ds4_session {
    ...
    ds4_kv_cache cuda_cpu_cache;
    ds4_cpu_decode_scratch cuda_cpu_scratch;
#endif
...
int ds4_engine_open(ds4_engine **out, const ds4_engine_options *opt) {
    ...
    if (e->backend == DS4_BACKEND_CUDA) {
#ifdef DS4_NO_CUDA
        fprintf(stderr, "ds4: CUDA backend requested but this build has no CUDA runtime support\n");
        ds4_engine_close(e);
        *out = NULL;
        return 1;
#else
        char err[256];
        ds4_cuda_info info;
        if (!ds4_cuda_init(&info, err, sizeof(err))) {
        ...
        e->cuda_ready = true;
        fprintf(stderr,
                "ds4: CUDA runtime initialized: device=%d/%d name=\"%s\" free=%.2f GiB total=%.2f GiB\n",
...
#endif
#ifndef DS4_NO_CUDA
    if (e->cuda_ready) ds4_cuda_cleanup();
#endif
...
int ds4_session_create(ds4_session **out, ds4_engine *e, int ctx_size) {
    if (!out || !e) return 1;
    *out = NULL;
    if (ctx_size <= 0) ctx_size = DS4_N_SWA;
#ifndef DS4_NO_CUDA
    if (e->backend == DS4_BACKEND_CUDA) {
        if (!e->cuda_ready) {
            fprintf(stderr, "ds4: CUDA session requested but CUDA runtime is not ready\n");
            return 1;
        }
        if (e->mtp_ready) {
            fprintf(stderr, "ds4: CUDA session does not support MTP yet\n");
            return 1;
        }
        ds4_session *s = xcalloc(1, sizeof(*s));
        s->engine = e;
        s->ctx_size = ctx_size;
        s->prefill_cap = cuda_graph_prefill_cap_for_context((uint32_t)ctx_size);
        char err[2048];
        if (!cuda_graph_alloc(&s->cuda_graph, &e->model, (uint32_t)ctx_size, err, sizeof(err))) {
        ...
        *out = s;
        fprintf(stderr,
                "ds4: CUDA session allocated (ctx=%d, prefill_chunk=%u, raw_kv_rows=%u, compressed_kv_rows=%u, mapped_weights=%.2f GiB)\n",
                ctx_size,
                s->cuda_graph.prefill_cap,
                s->cuda_graph.raw_cap,
                s->cuda_graph.comp_cap,
                (double)s->cuda_graph.model_weight_bytes / (1024.0 * 1024.0 * 1024.0));
        return 0;
    }
#endif
#ifdef DS4_NO_METAL
...
int ds4_session_eval_speculative_argmax(ds4_session *s, int first_token,
    ...
    if (s && s->engine && s->engine->backend == DS4_BACKEND_CUDA) {
        ...
        (void)accepted;
        (void)accepted_cap;
        return -cuda_graph_unimplemented(err, errlen, "speculative decode");
    }
#endif
#ifdef DS4_NO_METAL
