...
static void usage(FILE *fp) {
    fprintf(fp,
        ...
        "  --backend NAME\n"
        "      Select backend explicitly: metal or cuda. Default: metal\n"
        "  --cuda\n"
        "      Use the CUDA backend. This requires a CUDA-enabled ds4 build.\n"
        "  --quality\n"
...
static server_config parse_options(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(arg, "-h") || !strcmp(arg, "--help")) {
            ...
            const char *backend = need_arg(&i, argc, argv, arg);
            if (!strcmp(backend, "metal")) {
                c.engine.backend = DS4_BACKEND_METAL;
            } else if (!strcmp(backend, "cuda")) {
                c.engine.backend = DS4_BACKEND_CUDA;
            } else {
                server_log(LOG_DEFAULT, "ds4-server: invalid backend: %s", backend);
                server_log(LOG_DEFAULT, "ds4-server: valid backends are: metal, cuda");
                exit(2);
            }
            ...
            server_log(LOG_DEFAULT, "ds4-server: server mode does not support the CPU backend");
            exit(2);
        } else {
...
int main(int argc, char **argv) {
    ...
    if (ds4_session_create(&session, engine, cfg.ctx_size) != 0) {
        server_log(LOG_DEFAULT, "ds4-server: failed to create %s session",
