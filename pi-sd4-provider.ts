...
const ENDPOINT_FILE = process.env.DS4_ENDPOINT_FILE ?? join(SUPPORT_DIR, "logs", "current-ds4.env");
...
function endpointValue(name: string, text: string): string | undefined {
	const match = text.match(new RegExp(`^${name}=([^\\n]*)`, "m"));
	if (!match) return undefined;
	return match[1]?.trim().replace(/^['"]|['"]$/g, "");
}

function endpointBaseUrl(): string | undefined {
	if (process.env.DS4_BASE_URL) return normalizeBaseUrl(process.env.DS4_BASE_URL);
	for (const file of [ENDPOINT_FILE, join(EXTENSION_ROOT, "logs", "current-ds4.env")]) {
		try {
			const text = readFileSync(file, "utf8");
			const value = endpointValue("OPENAI_BASE_URL", text);
...
const EXTERNAL_BASE_URL = endpointBaseUrl();
const BASE_URL = EXTERNAL_BASE_URL ?? "http://127.0.0.1:8000";
const API_BASE_URL = `${BASE_URL}/v1`;
const USE_EXTERNAL_ENDPOINT = !!EXTERNAL_BASE_URL || process.env.DS4_MANAGED_SERVER === "0";
const SERVER_ARGS = (() => {
	const args = [
		"--ctx",
		process.env.DS4_CTX ?? "1000000",
		"--kv-disk-dir",
		KV_DIR,
		"--kv-disk-space-mb",
		process.env.DS4_KV_DISK_SPACE_MB ?? "32768",
	];
	if (process.env.DS4_SERVER_BACKEND) args.unshift("--backend", process.env.DS4_SERVER_BACKEND);
	return args;
})();
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("before_provider_request", async (_event, ctx) => {
		...
		if (USE_EXTERNAL_ENDPOINT) {
			if (await checkHttpReady()) return;
			const source = process.env.DS4_BASE_URL ? "DS4_BASE_URL" : ENDPOINT_FILE;
			const error = `ds4 endpoint is not ready at ${API_BASE_URL}; source=${source}`;
