...
### Repo-local Slurm RTX endpoint

The Slurm/Pi workflow in this checkout keeps its runtime files under this
repository:

```sh
cd /project/inniang/v4flash
./start-ds4-pi.sh
```

`start-ds4-pi.sh` is the daily entrypoint. It checks for `./ds4flash.gguf`,
downloads q2 into `./gguf/` when the link is missing, submits
`./serve-slurm-rtx.sh`, waits for `/v1/models`, verifies that the Pi provider
advertises `xhigh` effort and a `1000000` token context window, runs a small
startup benchmark, and prints the exact environment to launch Pi:

```sh
export DS4_ENDPOINT_FILE=/project/inniang/v4flash/logs/current-ds4.env
export DS4_BASE_URL=http://<slurm-node>:8000/v1
...
```sh
./start-ds4-pi.sh --status
./start-ds4-pi.sh --stop
./start-ds4-pi.sh --cuda-smoke
./start-ds4-pi.sh --no-download
./endpoint.sh --export
```

Runtime files stay in this checkout: `./gguf/` for model files, `./kv/` for
...
`start-ds4-pi.sh` writes each startup benchmark to
`./logs/benchmarks/ds4-rtx-<timestamp>.json`. Record confirmed runs here:

| Date | Commit | GPU | Quant | Context | Prompt Tokens | Output Tokens | Decode tok/s | Notes |
| --- | --- | --- | --- | ---: | ---: | ---: | ---: | --- |
| 2026-05-08 | local worktree | RTX 6000 Ada x2 | q2 | 1000000 | 0 | 0 | not run | CUDA smoke passed in Slurm job 70483: driver initialized, GGUF mapped, CUDA session scratch allocated; tok/s still blocked by missing CUDA graph kernels |
| 2026-05-08 | local worktree | login-node probe | q2 | 1000000 | 0 | 0 | not run | q2 weight present at `./ds4flash.gguf` (81G); login node has no `libcuda.so.1` |
